...
class BlueAgent:
    ...
    def moveBlue(self,p):   # imitates the red move, besides the subscriptions
        ...
        if isBlueAI == True:            # if any form of blue AI is competing
            if self.greyAgents > 0:     # and agents are present, then consider making an agent move
                ...
                if opinionlvl < 0.5:
                    ...
                    # print('GREY CHANCE') # grey delete
                    # print(str(greyOdds) + ' ' + str(1.0-greyOdds)) # grey delete
                    greyChance = random.choices([True, False], [greyOdds, 1.0-greyOdds])
                    if greyChance[0] == True:
                        # print('GREY IN ACTION') # grey delete
                        self.moveGrey()
                        return
...
def openGame():
    ...
    questions = [
        inquirer.Text('Population', message='How big is the Green population? (tested mostly at 20-30): ',
    ...
    questions = [
        inquirer.Text('scaling', message='State your lossScaling for the game. Bigger number = less affect. Default for (-1&1) intervals recommend is 2.5: ',
            validate=intervalBlue_validation)]
